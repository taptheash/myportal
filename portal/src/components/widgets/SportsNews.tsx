...
export default function SportsNews({ config, onUpdateConfig }: SportsNewsProps) {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        setLoading(true);
        const items = await fetchMergedRssWithCache('sportsnews', SOURCES, 2, articleCount, 3600000);
        setArticles(items);
        setError(null);
  ...
  if (loading) return <div className="flex items-center justify-center h-24"><div className="animate-spin rounded-full h-6 w-6 border-b-2 border-blue-500"></div></div>;
        ...
        <a key={idx} href={article.link} target="_blank" rel="noopener noreferrer"
          ...
          <div className="flex justify-between items-start gap-2">
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-gray-900 dark:text-white text-sm line-clamp-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition">{article.title}</p>
